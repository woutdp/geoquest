        ...
        <div transition:fly={{y: -100, duration: 500}} class="container absolute z-30 flex flex-col items-center justify-center mt-4 pointer-events-none">
            <div class="flex">
                <div class="flex py-2 rounded-md shadow-md pointer-events-auto bg-background-dark">
                    <span class="flex items-center justify-center mx-2 text-red">
                        <IconMistake />
                        {#key mistakes}
                            <span in:scale={{start: 1.5}} class="ml-1">{mistakes}</span>
                        {/key}
                    </span>
                    ...
                    {#key streak}
                        {#if streak > 1}
                            <span class="flex items-center justify-center mr-2 font-bold">
                                <span in:scale={{start: 1.5}} class="ml-1">{streak}x</span>
                            </span>
                        {/if}
                    {/key}
                </div>
            </div>
            <div transition:fly={{y: 20, duration: 500, delay: 500}}>
                {#key questionFeature}
            ...
            {#if arrowRotation}
                <div
                    ...
                    out:fly={{y: 20, duration: 2500, easing: quartOut}}
